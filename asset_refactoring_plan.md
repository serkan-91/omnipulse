...
* [ ] **`AssetType` Enum Tanımı:**
  * Dosya: `src/Modules/IoTModule/Domain/Enums/AssetType.cs`
...
* [ ] **`Asset` Sınıfının Oluşturulması (Eski `Vehicle` yerine):**
  * Dosya: `src/Modules/IoTModule/Domain/Entities/Asset.cs`
  * Alanlar: 
...
* [ ] **`Device` Sınıfının Güncellenmesi:**
  * Dosya: [Device.cs](src/Modules/IoTModule/Domain/Entities/Device.cs)
...
* [ ] **EF Core Konfigürasyonlarının Güncellenmesi:**
  * Dosya: `src/Modules/IoTModule/Infrastructure/Persistence/Configurations/AssetConfiguration.cs` (Eski `VehicleConfiguration` yerine oluşturulacak).
  * `MetadataJson` alanı için PostgreSQL JSON/JSONB eşleşmesi veya string dönüşümü tanımlanacak.
* [ ] **`IoTDbContext` Güncellemesi:**
  * Dosya: [IoTDbContext.cs](src/Modules/IoTModule/Infrastructure/Persistence/IoTDbContext.cs)
...
* [ ] **Göç (Migration) İşlemleri:**
  * Terminalden yeni bir migration üretilecek:
    ```bash
    dotnet ef migrations add TransitionToUnifiedAssetModel --project src/Modules/IoTModule/IoTModule.csproj --startup-project src/OmniPulse.Identity.API/OmniPulse.Identity.API.csproj --context IoTDbContext
    ```
...
* [ ] **`CreateVehicle` $\rightarrow$ `CreateAsset` Refaktörü:**
  * Klasör: `src/Modules/IoTModule/Features/Vehicles/CreateVehicle` ismi `Features/Assets/CreateAsset` yapılacak.
...
* [ ] **Sürücü/Operatör Telemetri Filtresi:**
  * Dosya: [TelemetryQueryExtensions.cs](src/Modules/IoTModule/Features/Telemetry/TelemetryQueryExtensions.cs)
...
* [ ] **Raporlama API'si:**
  * Dosya: [GetTelemetryReportQueryHandler.cs](src/Modules/IoTModule/Features/Telemetry/GetTelemetryReport/GetTelemetryReportQueryHandler.cs)
